...
async def test_reset_fl_data_dry_run_does_not_delete() -> None:
    ...
    with patch(
        "app.operations.fl_reset.get_session",
    ) as mock_get_session:
        session = AsyncMock()
        mock_get_session.return_value.__aenter__.return_value = session
        with patch(
            "app.operations.fl_reset.count_fl_scoped_rows",
            AsyncMock(return_value=expected),
        ) as mock_count:
            result = await reset_fl_data(dry_run=True)
    assert result == expected
    mock_count.assert_awaited_once_with(session)
...
async def test_reset_fl_data_deletes_in_order() -> None:
    ...
    with patch("app.operations.fl_reset.get_session") as mock_get_session:
        session = AsyncMock()
        mock_get_session.return_value.__aenter__.return_value = session
        with patch(
            "app.operations.fl_reset.count_fl_scoped_rows",
